...
w("<font size=3><b>工作站设定:</b></font><br>");
...
for(sid=0;sid<nboards*8;sid++) {
  ...
  else if (mas>0) w("<input type=checkbox "+(masop[bid]&(1<<s)?"checked":"")+" id=mc"+sid+">激活主站?");
  w("<br>");
}
...
w("</form></span>");
w("<button style=\"height:36\" onclick=\"fsubmit(sf)\">"+imgstr("submit")+"<b>提交变动</b></button>");
w("<button style=\"height:36\" onclick=\"rst()\">"+imgstr("reset")+"重命名</button>");
w("<button style=\"height:36\" onclick=\"fcancel()\">"+imgstr("delall")+"取消</button>");
