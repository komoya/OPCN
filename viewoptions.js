...
w("<b>参数设置</b>:<br><font size=2>(移动到每个选项查看提示)</font></div>");
w("<p></p>");
...
for(oid=0;oid<nopts;oid++){
  ...
  if(isbool)  w("<p title=\""+str_tooltips[oid]+"\"><b>"+name+"</b> <input type=checkbox "+(value>0?"checked":"")+" name=o"+index+">");
    ...
    if (name=="时区:") {
      ...
      for(i=1;i<=8;i++) w("<option "+(value==i?" selected ":" ")+"value="+i+">工作站 0"+i+"</option>");
      w("</select>");
    } else if (name=="HTTP 端口:") {
...
w("<h4>密码:<input type=password size=10 "+(opts[pasoid*4+2]?"disabled":"")+" name=pw></h4>");
w("<button style=\"height:36\" onclick=\"submit_form(of)\">"+imgstr("submit")+"<b>提交设置/b></button>");
w("<button style=\"height:36\" onclick=\"fcancel();return false;\">"+imgstr("delall")+"取消</button>");
w("<h4>修改密码</b>:<input type=password size=10 name=npw>&nbsp;&nbsp;Confirm:&nbsp;<input type=password size=10 name=cpw></h4>");
w("</form>");
