...
w("<button style=\"height:44\" onclick=\"fcancel()\">"+imgstr("back")+"后退</button>");
w("<button style=\"height:44\" onclick=\"mod(mf,-1)\">"+imgstr("addall")+"<b>添加新指令</b></button>");
w("<button style=\"height:44\" onclick=\"del(df,-1)\">"+imgstr("delall")+"删除所有指令</button>");
w("<button style=\"height:44\" onclick=\"fplot()\">"+imgstr("preview")+"预览</button><hr>");
w("<b>所有指令数量: "+nprogs+" (最大数量是 "+mnp+")</b><br>");
// print programs
...
for(pid=0;pid<nprogs;pid++) {
  ...
  w("</span>");
  // print buttons
  w("<br><button style=\"height:28\" onclick=del(df,"+pid+")>删除</button>");
  w("<button style=\"height:28\" onclick=mod(mf,"+pid+")>改动</button>");
  w("<hr>");
}
